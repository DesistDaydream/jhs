...
public abstract class Draft {
    ...
    public static HandshakeBuilder translateHandshakeHttp(ByteBuffer byteBuffer, Role role) throws InvalidHandshakeException {
        ...
        if (readStringLine != null) {
            String[] split = readStringLine.split(ExpandableTextView.N, 3);
            if (split.length == 3) {
                if (role == Role.CLIENT) {
                ...
                while (readStringLine2 != null && readStringLine2.length() > 0) {
                    String[] split2 = readStringLine2.split(Constants.COLON_SEPARATOR, 2);
                    if (split2.length == 2) {
    ...
    public int checkAlloc(int i2) throws InvalidDataException {
        if (i2 >= 0) {
            return i2;
        }
