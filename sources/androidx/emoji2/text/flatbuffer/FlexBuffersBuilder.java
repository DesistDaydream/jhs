...
public class FlexBuffersBuilder {
    ...
    public static class Value {
        ...
        private int storedWidth(int i2) {
            if (FlexBuffers.isTypeInline(this.type)) {
                return Math.max(this.minBitWidth, i2);
            }
    ...
    private void putUInt(String str, long j2) {
        ...
        if (widthUInBits == 0) {
            uInt64 = Value.uInt8(putKey, (int) j2);
        } else if (widthUInBits == 1) {
            uInt64 = Value.uInt16(putKey, (int) j2);
        } else if (widthUInBits == 2) {
            uInt64 = Value.uInt32(putKey, (int) j2);
        } else {
            uInt64 = Value.uInt64(putKey, j2);
        }
    ...
    public void putInt(String str, long j2) {
        int putKey = putKey(str);
        if (-128 <= j2 && j2 <= 127) {
            this.stack.add(Value.int8(putKey, (int) j2));
        } else if (-32768 <= j2 && j2 <= 32767) {
            this.stack.add(Value.int16(putKey, (int) j2));
        } else if (-2147483648L <= j2 && j2 <= 2147483647L) {
            this.stack.add(Value.int32(putKey, (int) j2));
        } else {
            this.stack.add(Value.int64(putKey, j2));
        }
    }
